...
(async function () {
	...
	const checkAndExecuteEvent = async () => {
		...
		//GET CURRENT TIME
		var timeVN = moment().tz('Asia/Kolkata').format('Kolkata:mm:ss');
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        ...
        return api.sendMessage(`✔ Approval K LiYe Request Admin Ko Send kr di Hai 🙂♥️\n\n vo reqst dekhte hi approve kar deng𝐢 💔🙂`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`Mujhe yaad krne ke liye dhanyavad` , event.threadID);
      }
    switch (event.type) {
      case "message":
      case "message_reply":
...
//THIS BOT WAS MADE BY ME(CATALIZCS) AND MY BROTHER SPERMLORD - DO NOT STEAL MY CODE (つ ͡ ° ͜ʖ ͡° )つ ✄ ╰⋃╯S MADE BY ME(CATALIZCS) AND MY BROTHER SPERMLORD - DO NOT STEAL MY CODE (つ ͡ ° ͜ʖ ͡° )つ ✄ ╰⋃╯