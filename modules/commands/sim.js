...
module.exports.run = async function ({ api: b, event: a, args: c }) {
    ...
    if (0 == c.length) return f("𝐉𝐢𝐢 𝐁𝐨𝐥𝐨 𝐌𝐞𝐫𝐢 𝐉𝐚𝐚𝐧 (◍•ᴗ•◍)❤");
    switch (c[0]) {
        case "on":
            return global.simsimi.has(d) ? f("You have not turned off the sim.") : (global.simsimi.set(d, e), f("Bol mai on ho gayi."));
        case "off":
            return global.simsimi.has(d) ? (global.simsimi.delete(d), f("Band ho gayi.")) : f("You have not enabled the sim.");
        default:
